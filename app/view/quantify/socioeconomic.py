...
def socioeconomic_facts_batch():
    if request.method == "POST":
        ...
        table = SocioeconomicTable.r_query().filter(SocioeconomicTable.id==table_id).first()
        if table is None:
...
def socioeconomic_facts_indexes(id):
    if request.method == "GET":
        table = SocioeconomicTable.r_query().filter(SocioeconomicTable.id==id).first()
...
def socioeconomic_table():
    ...
    if request.method == "DELETE":
        ...
        table = SocioeconomicTable.r_query().filter(
            SocioeconomicTable.id==request.form.get("id")).first()
...
def socioeconomic_index():
    ...
    if request.method == "PUT":
        index = SocioeconomicIndexes.r_query().filter(
            SocioeconomicIndexes.id==request.form.get("id")).first()
